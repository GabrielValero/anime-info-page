		...
		<div>
			...
			<div className="datos d-flex justify-content-center mt-5 mx-xl-5">
				<div className="col-xl-3 details">
					<iframe src={`https://www.youtube.com/embed/${anime.attributes.youtubeVideoId}`} width="100%" height="200px"></iframe>
					<div className="detalle">
						...
						<h4>Status</h4>
						<p className="m-0">{anime.attributes.status}</p>
					</div>
				</div>
				<div className="col-lg-8 d-flex justify-content-center flex-wrap">
					<Episodes episodes={episodes} title={anime.attributes.canonicalTitle}/>
				</div>
			</div>
			<style jsx>{`
				h1,h2,p{
...
Series.getInitialProps = async({query})=>{
	...
	const anime = await fetch(`https://kitsu.io/api/edge/anime/${id}`,{
	    headers:{
	...
	const episodes = await fetch(anime.relationships.episodes.links.related,{
	    headers:{
	      "Accept": "application/vnd.api+json",
	      "Content-Type": "application/vnd.api+json"
	    }
